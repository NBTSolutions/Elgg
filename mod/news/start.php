...
function news_init() {
	...
	$item = new ElggMenuItem('news', elgg_echo('news:blogs'), 'news/all');
    ...
    elgg_register_js('jquery.masonry', 'mod/news/vendors/masonry/jquery.masonry.min.js');
	// routing of urls
...
function news_page_handler($page) {
    ...
    if (elgg_get_plugin_setting('enable_masonry', 'news') == 'yes'){
        elgg_load_js('jquery.masonry');
        elgg_extend_view('page/elements/foot', 'news/js/mansonry');
    }
	...
	elgg_push_breadcrumb(elgg_echo('news:blogs'), "news/all");
 ...
 * @param array  $params  Parameters about the news posted
 * @return string
 */
function news_notify_message($hook, $type, $message, $params) {
	$entity = $params['entity'];
	...
	if (elgg_instanceof($entity, 'object', 'news')) {
		...
		return elgg_echo('news:notification', array(
			$owner->name,
			$title,
			$descr,
			$entity->getURL()
...
function news_ecml_views_hook($hook, $entity_type, $return_value, $params) {
	$return_value['object/news'] = elgg_echo('news:blogs');
...
function news_run_upgrades($event, $type, $details) {
	$news_upgrade_version = elgg_get_plugin_setting('upgrade_version', 'blogs');
...
function news_homepage() {
    ...
    $news = news_get_latest();

    if (!$news) {
        return elgg_echo('news:none');
    } else {
        return $news[0];
    }
