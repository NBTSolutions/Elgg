...
function news_get_page_content_list($container_guid = NULL) {
   $element_limit = elgg_get_plugin_setting('element_limit', 'news');
    if (!is_numeric($element_limit) || $element_limit <= 0) {
        $element_limit = 10;
    }
	...
	if ($container_guid) {
		...
		$return['title'] = elgg_echo('news:title:user_blogs', array($container->name));
		...
		$return['title'] = elgg_echo('news:title:all_blogs');
		elgg_pop_breadcrumb();
		elgg_push_breadcrumb(elgg_echo('news:blogs'));
	}
