...
if (($user instanceof ElggUser) && ($group instanceof ElggGroup)) {
	...
	if ($join) {
		if (investigations_join_investigation($group, $user)) {
			system_message(elgg_echo("investigations:joined"));
			forward($group->getURL());
		} else {
			register_error(elgg_echo("investigations:cantjoin"));
		}
	} else {
		...
		$subject = elgg_echo('investigations:request:subject', array(
			$user->name,
			$group->name,
		));
		$body = elgg_echo('investigations:request:body', array(
		...
		if (notify_user($group->owner_guid, $user->getGUID(), $subject, $body)) {
			system_message(elgg_echo("investigations:joinrequestmade"));
		} else {
			register_error(elgg_echo("investigations:joinrequestnotmade"));
		}
	}
} else {
	register_error(elgg_echo("investigations:cantjoin"));
