...
if (count($user_guids) > 0 && elgg_instanceof($group, 'group') && $group->canEdit()) {
	foreach ($user_guids as $guid) {
		...
		if (check_entity_relationship($group->guid, 'invited', $user->guid)) {
			register_error(elgg_echo("investigations:useralreadyinvited"));
			continue;
		}
		...
		$result = notify_user($user->getGUID(), $group->owner_guid,
				elgg_echo('investigations:invite:subject', array($user->name, $group->name)),
				elgg_echo('investigations:invite:body', array(
					$user->name,
					$logged_in_user->name,
					$group->name,
					$url,
				...
				NULL);
		if ($result) {
			system_message(elgg_echo("investigations:userinvited"));
		} else {
			register_error(elgg_echo("investigations:usernotinvited"));
		}
	}
}
