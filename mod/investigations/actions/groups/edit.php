...
elgg_make_sticky_form('investigations');
...
if ($is_new_group
		&& (elgg_get_plugin_setting('limited_groups', 'investigations') == 'yes')
		&& !$user->isAdmin()) {
	register_error(elgg_echo("investigations:cantcreate"));
	forward(REFERER);
}

$group = new ElggGroup($group_guid); // load if present, if not create a new group
if ($group_guid && !$group->canEdit()) {
	register_error(elgg_echo("investigations:cantedit"));
	forward(REFERER);
}
...
if (sizeof($input) > 0) {
	foreach($input as $shortname => $value) {
		...
		if (!$is_new_group && $shortname == 'name' && $value != $group->name) {
			...
			$ac_name = sanitize_string(elgg_echo('investigations:group') . ": " . $group_name);
...
// Validate create
if (!$group->name) {
	register_error(elgg_echo("investigations:notitle"));
	forward(REFERER);
}
...
system_message(elgg_echo("investigations:saved"));
