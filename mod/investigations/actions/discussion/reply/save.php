...
if (!$group->canWriteToContainer()) {
	register_error(elgg_echo('investigations:notmember'));
	forward(REFERER);
}
...
if ($annotation_id) {
	...
	if (!$annotation->canEdit()) {
		register_error(elgg_echo('investigations:notowner'));
		forward(REFERER);
	}

	$annotation->value = $text;
	if (!$annotation->save()) {
		system_message(elgg_echo('investigations:forumpost:error'));
		forward(REFERER);
	}
	system_message(elgg_echo('investigations:forumpost:edited'));
} else {
